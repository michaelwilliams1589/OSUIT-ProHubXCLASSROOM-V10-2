(() => {
  ...
  async function deletePersonFromProHub(uid, name, email) {
    ...
    // Do not scan /courseMembers. This keeps deletion independent of
    // course-wide read permissions and allows the core account removal to succeed.
    await db.ref().update(updates);
    ...
    return {
      ...
      accountBlocked: true,
      courseMembershipCleanupSkipped: true
    };
  }
  ...
  function confirmDeletePerson(uid, name, email) {
    modal(`<h2>Delete person from Pro Hub?</h2>
      ...
      <p>This removes the selected Pro Hub profile, role, calendar, notebooks, and personal dashboard data. It also blocks this UID from returning to this workspace. Existing course membership records are left untouched so deletion cannot be blocked by course permissions.</p>
    ...
    button.onclick = async () => {
      try {
        ...
        closeModal();
        toast(result.courseMembershipCleanupSkipped ? "Person removed. Course membership cleanup was skipped." : "Person removed from Pro Hub");
        renderView();
      } catch (error) {
